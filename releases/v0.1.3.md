...
### 🔒 Flatpak Sandbox Correctness & Host Path Routing
- **Host Override Routing**: Overrides created while running inside the Flatpak sandbox now write directly to the user's host application directory (`~/.local/share/applications/` via `HOST_XDG_DATA_HOME`), ensuring customized icons and names take effect immediately on your desktop.
- **Icon Suffix Compatibility**: Aligned icon resolution with FreeDesktop and GIO standards by automatically normalizing `.png`, `.svg`, and `.xpm` suffixes to theme icon names.
- **Safe Sandbox Boundaries**: Host and system applications that cannot be executed directly from within the sandbox have their Launch button gracefully disabled with an informative explanation tooltip.
- **Error Boundaries**: Bulk revert actions now feature transactional error handling, displaying clear error feedback if an operation is interrupted.
...
| **Arch Linux (Pacman)** | `any` | `glyph-0.1.3-1-any.pkg.tar.zst` |
| **Arch Linux (PKGBUILD)** | `any` | [`packaging/aur/PKGBUILD`](packaging/aur/PKGBUILD) |
