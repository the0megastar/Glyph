...
|---|---|---|---|---|---|
| **R1** | RPM relocation loop expands while writing spec | **Fixed & Verified** | `packaging/rpm/glyph.spec`, `.github/workflows/release.yml`, `.github/workflows/ci.yml`. Quoted heredoc, replaced Ubuntu dist-packages with Fedora `%python3_sitelib` and checked-in spec. | Verified live in CI: Fedora container built RPM and clean installed outside source tree. | None. Passed in GitHub Actions run [34063815107](https://github.com/the0megastar/Glyph/actions/runs/34063815107). |
| **R2** | Standalone bundled AppImage for x86_64 & ARM64 | **Fixed & Verified** | `scripts/build_appimage.sh`, `.github/workflows/release.yml`, `.github/workflows/ci.yml`. Stages Python, PyGObject (`gi`), GTK4, Libadwaita, typelibs, schemas, and robust `AppRun`. | Verified live in CI: AppImage bundles generated and tested on both x86_64 and aarch64 runners. | None. Passed in GitHub Actions run [34063815107](https://github.com/the0megastar/Glyph/actions/runs/34063815107). |
| **R3** | Mandatory PyGObject build dependencies declared | **Fixed & Verified** | `meson.build`, `packaging/rpm/glyph.spec`, `packaging/aur/PKGBUILD`, `.github/workflows/ci.yml`, `.github/workflows/release.yml`. Added `python3-gi`, `python3-gi-cairo`, `libgirepository1.0-dev`. | Verified in package definitions and CI workflow steps across Fedora, Arch, and Ubuntu. | None. |
...
| **R10** | Tests not connected to CI/release; documentation missing | **Fixed & Verified** | `meson.build`, `.github/workflows/ci.yml`, `.github/workflows/release.yml`, `docs/TESTING.md`, `docs/RELEASE_v0.1.3.md`, `docs/PERMISSIONS_AUDIT.md`. Tests registered in Meson and GitHub Actions. | `validate_release.py` passes; all 35 tests passing on Ubuntu, Fedora, and Arch runners. | None. Passed in GitHub Actions run [34063815107](https://github.com/the0megastar/Glyph/actions/runs/34063815107). |
...
| **5** | Correct RPM installation layout | **Fixed & Verified** | `packaging/rpm/glyph.spec`. Standard Fedora `%python3_sitelib`. | Verified live in CI: clean RPM build and install in GitHub Actions run [34063815107](https://github.com/the0megastar/Glyph/actions/runs/34063815107). | None. |
...
| **9** | Portable AppImage | **Fixed & Verified** | `scripts/build_appimage.sh`. True runtime bundle with libraries, typelibs, schemas. | Verified live in CI: bundles produced and tested for x86_64 & aarch64 in GitHub Actions run [34063815107](https://github.com/the0megastar/Glyph/actions/runs/34063815107). | None. |
